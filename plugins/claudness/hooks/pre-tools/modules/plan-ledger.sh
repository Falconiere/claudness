...
. "$_claudness_lib/plan-ledger-parse.sh"
...
ledger=$(pl_read_ledger "$state_file") || {
  jq -n --arg file "$state_file" '{
    "hookSpecificOutput": {
      ...
      "permissionDecisionReason": ("plan-ledger: unparseable ledger at " + $file + "; delete and re-run `bash plugins/claudness/hooks/lib/plan-ledger.sh run <plan_doc>`")
    }
  }'
  exit 0
}
...
if [[ "$version" != "1" ]]; then
  jq -n --arg file "$state_file" --arg v "$version" '{
    "hookSpecificOutput": {
      "hookEventName": "PreToolUse",
      "permissionDecision": "deny",
      "permissionDecisionReason": ("plan-ledger: ledger schema mismatch at " + $file + " (version=\"" + $v + "\", expected 1); delete and re-run `bash plugins/claudness/hooks/lib/plan-ledger.sh run <plan_doc>`")
    }
  }'
  exit 0
fi
...
reason=$(printf 'plan-ledger: push blocked — steps not fresh-green:\n%s\n\nrun: bash plugins/claudness/hooks/lib/plan-ledger.sh run %s' "$blockers" "$plan_doc")
