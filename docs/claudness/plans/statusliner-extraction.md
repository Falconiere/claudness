# Plan — Extract the statusline into a standalone `statusliner` plugin
...
optionally-installed plugin `statusliner@falconiere` so the statusline is an independent
...
- **Move, don't rewrite.** `git mv` the script and its tests so history is preserved.
  ...
  byte-untouched.
- **Fresh stable path, no backward compat.** statusliner owns its own namespace dir:
  `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/statusliner/statusline.sh`. Wiring command becomes
  `bash ~/.claude/statusliner/statusline.sh`. The old `claudness/` path is abandoned.
- **Zero dependencies.** statusliner declares no `dependencies` in marketplace.json /
...
- **Single symlinked file, no `.d/` registry.** statusline is not a hook event, so unlike
  `lang-quality/hooks/register.sh` (which mirrors `*.d/*.sh` under `<spec>__name`), statusliner
  ...
  (`${CLAUDE_CONFIG_DIR:-$HOME/.claude}/statusliner`) so there is no claudness lib dependency.
...
1. **Skeleton.** Create `plugins/statusliner/.claude-plugin/plugin.json` mirroring
   `lang-quality/.claude-plugin/plugin.json` shape: `name: statusliner`, `version: 0.1.0`,
...
2. **Move the script** (`git mv plugins/claudness/statusline.sh plugins/statusliner/statusline.sh`),
   then comment-only edits inside it:
   - line 2 title `Claudness statusline.` → `Statusliner statusline.`
...
3. **Move the tests** (`git mv plugins/claudness/__tests__/statusline.bats
   plugins/statusliner/__tests__/statusline.bats`). The suite resolves
   `SL="${BATS_TEST_DIRNAME}/../statusline.sh"`, so co-located under `statusliner/__tests__/`
   it still points at `statusliner/statusline.sh`. No test-body edits. CI auto-discovers it
...
3a. **Relocate the symlink test (BLOCKER from plan-review).** `plugins/claudness/hooks/__tests__/session-start.bats`
   ...
   The behavior moves to statusliner, so add its replacement in step 4b. `dep-warning.bats` is
   unaffected (it exercises the dependency-WARN block, not the symlink).

4. **statusliner hook wiring** — two new files:
   - `plugins/statusliner/hooks/hooks.json`: copy lang-quality's shape, SessionStart matcher
     `startup|resume|clear|compact`, command `${CLAUDE_PLUGIN_ROOT}/hooks/session-start.sh`.
   - `plugins/statusliner/hooks/session-start.sh`: consume stdin (`cat >/dev/null`), resolve
     `statusline_src` via `$(cd "$(dirname "$0")/.." && pwd)/statusline.sh`, set
     `reg_root="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/statusliner"`, `mkdir -p`, then the
     `-L`-or-absent guarded `ln -sf` to `$reg_root/statusline.sh`. `exit 0`. Concise doc
     header. Mark executable (`chmod +x`).

4b. **statusliner session-start test (replaces the relocated case from 3a).** New
   `plugins/statusliner/hooks/__tests__/session-start.bats`, real-data/no-mocks, mirroring the
   deleted claudness case but for the fresh path: run the hook with `CLAUDE_CONFIG_DIR="$TMP/cfg"`
   and `</dev/null`, assert (i) `$TMP/cfg/statusliner/statusline.sh` is a symlink (`-L`), (ii)
...
5. **statusliner README** — `plugins/statusliner/README.md`: what the line shows, the wiring
   snippet (`bash ~/.claude/statusliner/statusline.sh`, plus the `$CLAUDE_CONFIG_DIR` variant),
   ...
   `statusliner@falconiere`, re-point settings.json from the old `claudness/` path).
...
6. **claudness relinquishes the statusline** — edit `plugins/claudness/hooks/session-start.sh`:
   ...
   sweep now that statusliner owns the statusline. Also remove the now-dead
...
7. **Register the plugin** — add a `statusliner` entry to `.claude-plugin/marketplace.json`
   `plugins[]` (after `lang-quality`): `name`, `source: ./plugins/statusliner`, description,
...
8. **Docs in claudness point outward:**
   - `plugins/claudness/settings/README.md` (lines 82–110): replace the Statusline section body
     with a short pointer — "the statusline moved to the `statusliner` plugin; install
     `statusliner@falconiere` and wire `bash ~/.claude/statusliner/statusline.sh`" — or remove
     it, deferring detail to statusliner/README.md.
   - `README.md`: line 62 plugin table — drop "and a gate-aware statusline" from the claudness
     row, add a `statusliner` row (`0.1.0`, gate-aware statusline). Line 122 "More that comes
     with it" — reword the Gate-aware statusline bullet to note it ships in `statusliner`. Lines
...
Create:
- `plugins/statusliner/.claude-plugin/plugin.json`
- `plugins/statusliner/hooks/hooks.json`
- `plugins/statusliner/hooks/session-start.sh`
- `plugins/statusliner/hooks/__tests__/session-start.bats` (step 4b)
- `plugins/statusliner/README.md`

Move (git mv):
- `plugins/claudness/statusline.sh` → `plugins/statusliner/statusline.sh`
- `plugins/claudness/__tests__/statusline.bats` → `plugins/statusliner/__tests__/statusline.bats`

Edit:
- `plugins/statusliner/statusline.sh` (comment-only, post-move)
...
- **Tests (real-data, no mocks — already the case):**
  `bats plugins/statusliner/__tests__/statusline.bats` → all 8 pass (model/ctx, effort
...
- **Lint:** `shellcheck plugins/statusliner/hooks/session-start.sh plugins/statusliner/statusline.sh
  plugins/claudness/hooks/session-start.sh` clean (repo has `.shellcheckrc`).
- **Symlink smoke test:** run `plugins/statusliner/hooks/session-start.sh </dev/null` with a
  temp `CLAUDE_CONFIG_DIR`; assert `$CLAUDE_CONFIG_DIR/statusliner/statusline.sh` exists, is a
  symlink, points at the plugin's `statusline.sh`, and that a pre-existing *real* file at that
  path is NOT clobbered.
- **Render smoke test:** `printf '{"model":{"display_name":"Opus"},"context_window":{"context_window_size":200000,"total_input_tokens":45000,"used_percentage":22}}' | bash ~/.claude/statusliner/statusline.sh`
...
- **Orphan cleanup:** create a symlink at `$CLAUDE_CONFIG_DIR/claudness/statusline.sh`, run the
  ...
  it survives.
- **JSON validity:** `jq . .claude-plugin/marketplace.json plugins/statusliner/.claude-plugin/plugin.json plugins/statusliner/hooks/hooks.json`.

## Open risk (document, don't fix)

Existing users who currently get the statusline auto-wired via claudness will lose it on
upgrade until they `/plugin install statusliner@falconiere` and re-point `settings.json` to
`bash ~/.claude/statusliner/statusline.sh`. The orphan-cleanup in step 6 removes the dangling
old symlink so it fails loudly (missing file) rather than silently pointing into a cleaned
cache. This breaking change is surfaced in `statusliner/README.md` and the claudness README/
