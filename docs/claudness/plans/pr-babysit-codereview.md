...
- `plugins/pr-babysit/scripts/__tests__/parse-verdict.bats` — assert pr31 fixture → `complete:true,verdict:approved`, **exactly 6 findings**, correct path/severity/text, stable keys; in-progress → `complete:false,state:in_progress`; no-checkbox → `state:unknown`.
...
- `[bats]` `bats plugins/pr-babysit/scripts/__tests__/parse-verdict.bats` — 6-findings parse + in-progress + no-checkbox (acceptance #2,#3).
